...
NODE_CONTAINER_ID=$( \
  docker build -t "$NODE_NAME" ./kafka_node/ | \
  tee /dev/tty)
ZK_CONTAINER_ID=$( \
...
for i in `seq 1 $BROKERS`; do
  cat kafka_node/server.properties | \
    sed "s/broker.id=0/broker.id=$i/" | \
    sed "s/172.39.39.11/$NODE_IP_PREFIX$i/" | \
    sed "s/9092/909$i/" > \
    "kafka_node/server.$i.properties"
  echo "Kafka node container ID: $( \
    docker run -dit \
      --name "$NODE_NAME-$i" \
      --net "$NETWORK_NAME" \
      --ip "$NODE_IP_PREFIX$i" \
      -v "$(pwd)/kafka_node/server.$i.properties:/usr/share/kafka_2.11-0.11.0.1/config/server.properties" \
      -p "909$i:909$i" \
      "$NODE_CONTAINER_ID")"
done
